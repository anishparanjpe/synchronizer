...
const char PATH_EXP[] = "path {(CaregiverArrive+ChildArrive+ChildLeave+CaregiverLeave)} end path {(CaregiverCountLock+ChildCountLock)} end";
...
void caregiver_arrive(_thread_info_t *myinfo)
{
  ...
  // YOUR CODE HERE
  ENTER_OPERATION("CaregiverCountLock");
  ca_cnt++;
  EXIT_OPERATION("CaregiverCountLock");
...
void caregiver_leave(_thread_info_t *myinfo)
{
  ...
  if (ca_cnt == 0)
    ...
    while (CAREGIVER_CAPACITY * ca_cnt > ch_cnt)
      ;
...
void child_arrive(_thread_info_t *myinfo)
{
  ...
  // YOUR CODE HERE
  ENTER_OPERATION("ChildCountLock");
  ch_cnt++;
  EXIT_OPERATION("ChildCountLock");

  if (ca_cnt == 0)
    while (ca_cnt == 0)
      ;
  else
    while (ch_cnt >= CAREGIVER_CAPACITY * ca_cnt + 1)
      ;
